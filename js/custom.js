...
function handleSubmit(e) {
    ...
    setTimeout(()=>{
        ...
        .then(data => {
            ...
            if (cigs>0 & prediction > 0.05){
                document.querySelector('.clearfix').innerHTML = `<p class="nl-form">Quit Smoking<br>Predicted probability of having a coronary heart disease (Heart Attack) is ${prediction} <br> Please consult Cardiologist<p>`;
            }
            else if (cigs>1 & prediction < 0.05){
                document.querySelector('.clearfix').innerHTML = `<p class="nl-form">Quit Smoking<br>Predicted probability of having a coronary heart disease (Heart Attack) is ${prediction}</p>`;

            }
            else {
                document.querySelector('.clearfix').innerHTML = `<p class="nl-form">Predicted probability of having a coronary heart disease (Heart Attack) is ${prediction}`;
            }
        })
        .catch(err => {
            console.log(err)
            document.querySelector('.loader').style.display = 'none';
            document.querySelector('.clearfix').innerHtml = `<p class="nl-form">Sorry, </p>`
        })
